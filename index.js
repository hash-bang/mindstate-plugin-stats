		...
		async()
			...
			.then(function(next) {
				...
				if (!mindstate.config.stats.enabled) {
					if (mindstate.verbose) console.log(colors.blue('[Stats]'), 'Stats backup is disabled');
					return next('SKIP');
				}
				if (!_.keys(mindstate.config.stats.commands).length) {
					if (mindstate.verbose) console.log(colors.blue('[Stats]'), 'Stats backup has no commands to run');
					return next('SKIP');
				}
				next();
				// }}}
				...
				async()
					...
					.then('binPath', function(next) {
						which(bin, function(err, path) {
							if (err) {
								if (mindstate.verbose > 1) console.log(colors.blue('[Stats]'), 'Stats binary command `' + bin + '` is not in PATH');
								return next('NOBIN');
							}
							next(null, path);
						});
